    ...
    try:
        ...
        if data.get('follow_up') and data.get('action') == 'add_event':
            ...
            if suggested_event:
                ...
                # Create event using user_id instead of credentials
        ...
        # Get data for AI assistant - no longer need credentials
        ...
        # Process commands
        if is_command:
            return process_command(command_type, command_content, user_id)
...
def process_command(command_type, command_content, user_id):
    """Process a command from the chatbot"""
    try:
        if command_type == "add_event":
            return add_event_command(command_content, user_id)
        elif command_type == "remove_event":
            return remove_event_command(command_content, user_id)
        elif command_type == "list_events":
            return list_events_command(user_id)
        elif command_type == "show_help":
            return show_help_command()
        elif command_type == "check_availability":
            return check_availability_command(command_content, user_id)
        elif command_type == "suggest_time":
            return suggest_time_command(command_content, user_id)
        else:
            return jsonify({
...
def add_event_command(command_content, user_id):
    ...
    try:
        ...
        email_id = None
        if 'https://outlook.live.com/mail/' in command_content:
...
def remove_event_command(command_content, user_id):
...
def list_events_command(user_id):
...
def check_availability_command(command_content, user_id):
...
def suggest_time_command(command_content, user_id):
